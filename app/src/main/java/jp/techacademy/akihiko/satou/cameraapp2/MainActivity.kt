package jp.techacademy.akihiko.satou.cameraapp2
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var recRequestBuilder     : CaptureRequest.Builder
    private lateinit var recRequest            : CaptureRequest
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        shutterButton.setOnClickListener {
            ...
            try {
                ...
                createCameraRecSession()
               // captureSession.setRepeatingRequest(recRequest, null, null)
    ...
    private fun createCameraRecSession()
    {
        try
        {
            val texture = previewView.surfaceTexture
            ...
            val surface = Surface(texture)
            recRequestBuilder = cameraDevice!!.createCaptureRequest(CameraDevice.TEMPLATE_RECORD)
            recRequestBuilder.addTarget(surface)
            ...
            cameraDevice?.createCaptureSession(Arrays.asList(surface, imageReader.surface),
                ...
                object : CameraCaptureSession.StateCallback()
                {
                    override fun onConfigured(cameraCaptureSession: CameraCaptureSession)
                    {
                        ...
                        {
                            captureSession = cameraCaptureSession
                            recRequestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE)
                            recRequest = recRequestBuilder.build()
                            cameraCaptureSession.setRepeatingRequest(recRequest, null, Handler(backgroundThread?.looper))
                        } catch (e: CameraAccessException) {
                            Log.e("erfs", e.toString())
                        }
